...
def test_pendulum_derivatives_correct(
    L: float, g: float, theta: float, omega: float
) -> None:
    """
...
# Equilibrium test
@pytest.mark.parametrize("L,g", [(1.0, 9.81), (0.75, 9.81), (2.0, 3.81)])
...
@pytest.mark.parametrize(
    "L,g,T,dt", [(1.0, 9.81, 5.0, 0.01), (2.0, 9.81, 3.0, 0.05), (0.5, 1.62, 2.0, 0.02)]
)
def test_solve_pendulum_function_zero_ic(
    L: float, g: float, T: float, dt: float
) -> None:
    """
    Run with:
...
def test_pendulum_energy_and_velocities(
    L: float, g: float, u0: float, T: float, dt: float
) -> None:
    """
    Run with:
